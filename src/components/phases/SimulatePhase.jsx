...
import BalanceLabStation from '../simulations/BalanceLabStation.jsx';
import ScaleReaderStation from '../simulations/ScaleReaderStation.jsx';
import MassConverterStation from '../simulations/MassConverterStation.jsx';
import RecipeInspectorStation from '../simulations/RecipeInspectorStation.jsx';
import { useAudio } from '../../hooks/useAudio.js';
import { simStationIntro } from '../../utils/narration.js';

const STATIONS = [
  { id: 0, label: 'A', name: 'Pan Balance Lab',       icon: '⚖️', desc: 'Place weights to balance' },
  { id: 1, label: 'B', name: 'Scale Dial Reader',     icon: '🧭', desc: 'Read dials & target totals' },
  { id: 2, label: 'C', name: 'kg ↔ g Converter',      icon: '🔄', desc: 'Convert grams and kilograms' },
  { id: 3, label: 'D', name: 'Recipe Mass Inspector', icon: '🧁', desc: 'Inspect recipe ingredient masses' },
];
    ...
    <div className="sim-wrap">
      <div className="sim-card glass-card">
        ...
        <div className="sim-station-area" role="tabpanel" key={s}>
          {s === 0 && <BalanceLabStation onComplete={() => handleStationComplete(0)} audioEnabled={state?.audioEnabled} />}
          {s === 1 && <ScaleReaderStation onComplete={() => handleStationComplete(1)} audioEnabled={state?.audioEnabled} />}
          {s === 2 && <MassConverterStation onComplete={() => handleStationComplete(2)} audioEnabled={state?.audioEnabled} />}
          {s === 3 && <RecipeInspectorStation onComplete={() => handleStationComplete(3)} audioEnabled={state?.audioEnabled} />}
        </div>
